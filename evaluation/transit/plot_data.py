...
def extract_directory(dirPath: str) -> dict:
    ...
    files = [f for f in files if f.endswith("_stats.txt") and "NO_EXT" in f]
    files.sort()
    ...
    for f in files:
        ...
        mode = re.search(r"_\d+_", f).group(0)
        mode = re.search(r"\d+", f).group(0)
